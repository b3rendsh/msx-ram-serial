...
; -----------------------------------------------------------------------------
; [MSX] changes made by H.J. Berends:
; + Modified sources to assemble with z88dk's z80asm
; + Adapted for MSX fossil driver v2
; + Shorter messages for 40 columns MSX1 screen
; + SETUSR and other USR routines or variables are ignored or not implemented
; + Adapted for MSX specific ALLOC routine to calculate free space 
;
; [WBW] include some (not all) of the changes by W.Warthen for ROMWBW version:
; + Parameter for port selection
; + Faster handshake to start transfer
; + Bug fixes
; -----------------------------------------------------------------------------
...
;
MHZ	EQU	3	; Clock speed, use integer (2,4,5,8, etc.)
...
MSX	EQU	YES	; [MSX] Yes, if operating with a MSX machine
;
;=======================================================================
;
...
;
MAXDRV	EQU	8	; Number of disk drives used (1=A, 2=B, etc)
...
;
	 IF	MTOS && MBMXT	; both must be YES
MXTOS	EQU	YES	; (leave YES)
	 ENDIF
;
	 IF	!(MTOS && MBMXT) ; (if either is NO)
...
;
	 IF	MXTL && MXTOS	; both must be YES
MTL	EQU	YES	; (leave YES)
	 ENDIF
;
	 IF	!(MXTL && MXTOS); (if either are NO)
MTL	EQU	NO	; (leave NO)
	 ENDIF
;
...
EXTRA3:	JMP	0		; Extra for custom routine
	 ...
	 IF	!(LARGEIO || BYEBDOS)
	ALIGN	100H+80H	; Origin plus 128 bytes for patches
	 ENDIF

	 IF	LARGEIO	&& !BYEBDOS
	ALIGN	100H+LARSIZE	; I/O patch area size if over 128 bytes
	 ENDIF

	 ELSE			; [MSX] Fossil module routines
	PUBLIC	BEGIN
...
;
	...
	DB	VMONTH/10+'0',(VMONTH % 10)+'0',"/"
	DB	VDAY/10+'0',(VDAY % 10)+'0',"/"
	DB	VYEAR/10+'0',(VYEAR % 10)+'0',CR,LF,0
;
...
SNDOPC:	INX	H		; Look for an option
	...
	JMP	ALLSET		; [WBW]
CHKK:	CPI	'K'
	JNZ	CHK6TH		; [WBW] If it's not K it should be a port number
	LDA	MSPEED
...
;
	...
	CALL	ILPRT
	 IF	MSX
	DB	CR,LF,"Upload files to named or current drive",0
	 ELSE
...
;
	...
	MVI	H,0
	MOV	L,A
	 IF	!MSX		; [MSX] USR not implemented
	CALL	DECOUT
	 ENDIF
	MVI	A,':'
	CALL	CTYPE
...
;
	...
	DB	"++ Examples of valid options: ++ ",CR,LF
	DB	"(use Ctrl-C or Ctrl-K to abort)",CR,LF,LF
;
; [MSX] Display shorter customized help message
	 IF	MSX
	DB	"XM S TEST.DOC   send a file to you",CR,LF
	DB	"XM S B:TEST.DOC send from a named drive",CR,LF
	DB	"XM SK TEST.DOC  send in 1k blocks",CR,LF
	DB	"Add 0 OR 1 to specify serial port",CR,LF,LF
	DB	"XM R TEST.DOC   receive a file from you", CR,LF
	DB	"XM RP TEST.DOC  receive in private area", CR,LF
	DB	"Add C for forced checksum: \"RC\" \"RPC\"",CR,LF
	DB	"Add X for forced 128 byte: \"RX\" \"RPX\"",CR,LF
	DB	"Add 0 OR 1 to specify serial port"
	DB	CR,LF,"$"
	 ELSE
	 IF	!LUXMOD
	DB	"XM S HELLO.DOC         send a file to you",CR,LF
	DB	"XM S B1:HELLO.DOC      send from a named drive/area",CR,LF
	DB	"XM SK HELLO.DOC        send in 1k blocks",CR,LF
	DB	"XM L CAT.LBR CAT.COM   send a file from a library",CR,LF
	DB	"XM LK CAT.LBR CAT.COM  send in 1k blocks",CR,LF
	DB	"   The \".LBR\" file extension may be omitted",CR,LF
	DB	"   Add \"0\"-\"9\" to specify serial port",CR,LF,LF
	DB	"XM R HELLO.DOC         receive a file from you", CR,LF
	DB	"XM RP HELLO.DOC        receive in a private area", CR,LF
	 ENDIF
;
	 IF	(MBDESC	|| DESCRIB) &&	NDESC
	DB	"XM RN FILE.EXT         receive without description"
	DB	CR,LF
	 ENDIF
;
	 IF	!LUXMOD && MBFMSG
	DB	"XM RM MESSAGE.FIL      receive message for MBBS"
	DB	CR,LF
	 ENDIF
;
	 IF	!LUXMOD
	...
	DB	CR,LF,LF
	DB	"XM A                   shows areas/space for uploads$"
	 ENDIF
;
	 IF	LUXMOD
...
BYECHK:	MVI	C,32		; This bizarre combination determines
	...
	RET
	 ENDIF
;
	 IF	!BYEBDOS && (USECON || ACCESS || WRTLOC)
...
;-----------------------------------------------------------------------
;
EXIT1:	 IF	(TIMEON	|| B3TOS) && !LUXMOD && TOSEXIT
	CALL	TIME		; Tell user how long he's been on
	 ENDIF
;
	 IF	(BYEBDOS && !TIMEON) && TOSEXIT && !LUXMOD
...
;
; [MSX] Make drive leter upper case
	 IF	MSX
	AND	$5F		
	 ENDIF
;
...
;
CPMVER	EQU	0CH		; [MSX] returns CP/M version 2.2
CURDPB	EQU	1FH		; [MSX] not implemented
GALLOC	EQU	1BH		; [MSX] not compatible
;SELDSK	EQU	0EH		; duplicate definition (Z88DK)
GETFRE	EQU	46		; [MSX] not compatible
;
...
;
; [MSX] 
; ALLOC function may return wrong value on large drives and take long to return.
; If using Nextor you can fix this with the RALLOC command.
	 IF	MSX		
FREE20:	LD	E,0		; Current drive
	LD	C,GALLOC
	CALL	BDOS		; HL=Free clusters, A=Sectors per cluster (HL*A/2 = Free KB)
	SRL	A		; A=A/2
	LD	B,A		; Multiplier
	DEC	B
	JR	Z,SAVFRE	; if 2 sectors per cluster then #kb = #clusters
	LD	D,H
	LD	E,L
FRADD:	ADD	HL,DE
	DJNZ	FRADD
...
;
	...
	DB	"at this time...",'$'
	 ENDIF
;
SNDFOK:	 IF	NOSYS && !LUXMOD
	LDA	FCB+10
	ANI	80H
	JNZ	NONAME		; If $SYS then fake a "file not found"
	 ENDIF
;
	 IF	OK2400 && TAGLBR && !LUXMOD
...
SKIPTIM:MOV	L,C
	...
	CALL	SPEED
	CPI	5
	MVI	A,'0'		; Adds a zero for bps 1200 and higher
	CNC	CTYPE	
	CALL	SPEED		; [MSX]
	CPI	9		; [MSX]
	MVI	A,'0'		; [MSX] Adds another zero for bps 19200 and higher
	CNC	CTYPE		; [MSX]
...
;
	...
	DB	" minutes allowed ++$"
	 ENDIF
;
; [MSX] Added factors (guesstimates) for 38400,57600 and 115200 bps 
BTABLE:	 IF	!STOPBIT	; One stop bit
	DW	5,13,19,25,30,48,85,141,210,280,360,480,700,0
	 ENDIF
;
	 IF	STOPBIT		; Two stop bits
	DW	5,12,18,23,27,44,78,128,191,255,320,400,620,0
	 ENDIF
;
KTABLE:	 IF	!STOPBIT	; One stop bit
	DW	5,14,21,27,32,53,101,190,330,525,750,1000,1800,0
	 ENDIF
;
	 IF	STOPBIT		; Two stop bits
	DW	5,13,19,25,29,48,92,173,300,477,600,900,1650,0
	 ENDIF
;
RECTBL:	 IF	!STOPBIT	; One stop bit
	DB	192,74,51,38,32,20,11,8,5,3,2,1,1,0
	 ENDIF
;
	 IF	STOPBIT		; Two stop bits
	DB	192,80,53,42,36,22,12,7,5,4,3,2,1,0
	 ENDIF
;
KECTBL:	 IF	!STOPBIT	; One stop bit
	DB	192,69,46,36,30,18,10,5,3,2,1,1,1,0
	 ENDIF
;
	 IF	STOPBIT		; Two stop bits
	DB	192,74,51,38,33,20,10,6,3,2,1,1,1,0
	 ENDIF
;
; [MSX] Added speed factors for 38400,57600 and 115200 bps
SPTBL:	DB	"110$","300$","450$","600$","710$","120$","240$"
	DB	"480$","960$","192$","382$","576$","1152$"
;
...
FULL:	MVI	C,DELET
	...
	CALL	DEXIT
	 IFDEF	MSX
	DB	CR,LF,"++ DISK FULL: ABORT & SAVE ORIGINAL ++$"
	 ELSE
...
RESET:	LDA	DRUSER		; Get original drive/user area back
	...
	MOV	E,A
	MVI	C,SETUSER	; Restore original user area
	CALL	BDOS
...
RECARE:	MOV	E,A		; Stuff it in E
	MVI	C,SETUSER	; Tell BDOS what we want to do
	CALL	BDOS		; Now do it
	RET
;
RECDRV:	LDA	PRVTFL
...
LOGC1:	MVI	C,SETRRD	; Get random record #
	...
	CALL	BDOS
	 ENDIF	;LOGCAL
;
	 IF	LOGCAL && !(MBBS && (RTC || B3RTC || BYEBDOS))
...
;
	...
	CALL	BDOS
	 ENDIF
;
	 IF	LOGCAL && OXGATE && (B3RTC || RTC || BYEBDOS)
	XRA	A
...
EOLF:	LHLD	FCBLOG+33
	...
	RET
;
; Open file with FCB pointed to by DE (disk/user passed in DEFAULT_DISK
; and DEFAULT_USER)
;
OPENF:	PUSH	D		; Save FCB address
	LDA	DEFAULT_DISK	; Get disk for file
	CALL	RECDRX		; Log into it
	LDA	DEFAULT_USER	; Get default user
...
;
	...
	JZ	TIME3
	 ENDIF
;
	 IF	TIMEON && !BYEBDOS ; BYEBDOS doesn't use status byte
	ORA	A		; Special user?
...
;-----------------------------------------------------------------------
;
GETDATE: IF	(RTC && LOGCAL) && !(CPM3 || BYEBDOS)
...
;	    B - current hour   (0-23)
;
GETTIME: IF	(TIMEON	|| RTC)	&& !(B3RTC || CPM3 || BYEBDOS)
;
;; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
BLKMAX:	DB	0,0
;
	 IF	B3RTC && !BYEBDOS	; If BYE3/MBYE real-time clock
...
SETUSER	EQU	32		; Set user area to receive file (Z88DK: SETUSR is a reserved word)
